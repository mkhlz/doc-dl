...
def write_build_info(bundle: Path, target: str, version: str) -> None:
    payload = {
        "format": 1,
        "name": "doc-dl",
        "target": target,
...
def build(target: str, browser_source: Path, output: Path, work_root: Path) -> Path:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Playwright browser directory; defaults to PLAYWRIGHT_BROWSERS_PATH",
    )
...
def main() -> int:
    ...
    if browser_dir is None:
        ...
        if not configured:
            raise RuntimeError("Set PLAYWRIGHT_BROWSERS_PATH or pass --browser-dir")
        browser_dir = Path(configured)
    asset = build(args.target, browser_dir, args.output.resolve(), args.work_root.resolve())
